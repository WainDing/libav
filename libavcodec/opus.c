    ...
    if (!map_type) {
        ...
        if (map_type == 1) {
            ...
            if (channels <= 227 && (channels == idx || channels == idx + 2)) {
                int ret = av_channel_layout_ambisonic(&avctx->ch_layout, channels);
                if (ret < 0)
                    return ret;

                /* ACN order, followed by two optional channels of non-diegetic stereo */
                if (idx != channels) {
                    avctx->ch_layout.u.map[idx + 0] = AV_CHAN_FRONT_LEFT;
                    avctx->ch_layout.u.map[idx + 1] = AV_CHAN_FRONT_RIGHT;
                }
            } else {
